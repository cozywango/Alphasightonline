...
export const AuthComponent = ({ logo = <DefaultLogo />, brandName = "Alphasight Online" }: AuthComponentProps) => {
    ...
    const handleFinalSubmit = async (e: React.FormEvent) => {
        ...
        try {
            if (isSignUp) {
                ...
                if (error) throw error;
                setModalStatus('success'); // Re-use success modal for redirect delay or just redirect immediately
            ...
            if (msg.includes("User already registered") || msg.includes("already registered")) {
                ...
                // Optional: Switch to Sign In mode automatically here if desired
            }
    ...
    const handleProgressStep = () => {
        if (authStep === 'email') {
            if (isEmailValid) {
                if (isSignUp) setAuthStep("username");
    ...
    const handleGoBack = () => {
        ...
        else if (authStep === 'username') setAuthStep('email');
    };
    ...
    useEffect(() => {
        if (authStep === 'username') setTimeout(() => usernameInputRef.current?.focus(), 500);
                            ...
                            <div className="flex flex-col items-center gap-4">
                                {isSignUp ? (
                                    <>
                                        ...
                                        <p className="text-sm text-muted-foreground text-center">Please check your email to confirm.<br /><span className="text-xs opacity-70">If account exists, you will receive a login link.</span></p>
                                    </>
                                ) : (
                                    <>
                ...
                <fieldset disabled={modalStatus !== 'closed'} className="relative z-10 flex flex-col items-center gap-8 w-[280px] mx-auto p-4">
                    <AnimatePresence mode="wait">
                        ...
                        {authStep === "username" && <motion.div key="username-title" initial={{ y: 6, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ opacity: 0 }} transition={{ duration: 0.3, ease: "easeOut" }} className="w-full flex flex-col items-center text-center gap-4">
                            ...
                            <BlurFade delay={0.25 * 1}><p className="text-sm font-medium text-muted-foreground">This is how you be known on the platform.</p></BlurFade>
                        </motion.div>}
                        {authStep === "password" && <motion.div key="password-title" initial={{ y: 6, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ opacity: 0 }} transition={{ duration: 0.3, ease: "easeOut" }} className="w-full flex flex-col items-center text-center gap-4">
                            <BlurFade delay={0} className="w-full"><div className="text-center"><p className="font-serif font-light text-4xl sm:text-5xl tracking-tight text-foreground whitespace-nowrap">{isSignUp ? "Create your password" : "Enter your password"}</p></div></BlurFade>
                            <BlurFade delay={0.25 * 1}><p className="text-sm font-medium text-muted-foreground">{isSignUp ? "Your password must be at least 6 characters long." : "Welcome back!"}</p></BlurFade>
                        </motion.div>}
                    ...
                    <form onSubmit={handleFinalSubmit} className="w-[300px] space-y-6">
                        <AnimatePresence>
                            {authStep !== 'confirmPassword' && <motion.div key="email-password-fields" exit={{ opacity: 0, filter: 'blur(4px)' }} transition={{ duration: 0.3, ease: "easeOut" }} className="w-full space-y-6">
                                ...
                                <AnimatePresence>
                                    {authStep === "username" && <BlurFade key="username-field" className="w-full">
                                        <div className="relative w-full">
                                            <AnimatePresence>
                                                {username.length > 0 && <motion.div initial={{ y: -10, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ duration: 0.3 }} className="absolute -top-6 left-4 z-10"><label className="text-xs text-muted-foreground font-semibold">Username</label></motion.div>}
                                            </AnimatePresence>
                                            <div className="glass-input-wrap w-full"><div className="glass-input">
                                                <span className="glass-input-text-area"></span>
                                                <div className="relative z-10 flex-shrink-0 flex items-center justify-center w-10 pl-2"><User className="h-5 w-5 text-foreground/80 flex-shrink-0" /></div>
                                                <input ref={usernameInputRef} type="text" placeholder="Username" value={username} onChange={(e) => setUsername(e.target.value)} onKeyDown={handleKeyDown} className="relative z-10 h-full w-0 flex-grow bg-transparent text-foreground placeholder:text-foreground/60 focus:outline-none" />
                                                <div className={cn("relative z-10 flex-shrink-0 overflow-hidden transition-all duration-300 ease-in-out", isUsernameValid ? "w-10 pr-1" : "w-0")}><GlassButton type="button" onClick={handleProgressStep} size="icon" aria-label="Continue with username" contentClassName="text-foreground/80 hover:text-foreground"><ArrowRight className="w-5 h-5" /></GlassButton></div>
                                            </div></div>
                                        </div>
                                    ...
                                    {authStep === "password" && <BlurFade key="password-field" className="w-full">
                                        <div className="relative w-full">
                                            ...
                                            <div className="glass-input-wrap w-full"><div className="glass-input">
                                                ...
                                                <div className={cn("relative z-10 flex-shrink-0 overflow-hidden transition-all duration-300 ease-in-out", isPasswordValid ? "w-10 pr-1" : "w-0")}><GlassButton type={isSignUp ? "button" : "submit"} onClick={isSignUp ? handleProgressStep : undefined} size="icon" aria-label="Submit password" contentClassName="text-foreground/80 hover:text-foreground"><ArrowRight className="w-5 h-5" /></GlassButton></div>
                                            </div></div>
                                        </div>
                                        <BlurFade inView delay={0.2}><button type="button" onClick={handleGoBack} className="mt-4 flex items-center gap-2 text-sm text-foreground/70 hover:text-foreground transition-colors"><ArrowLeft className="w-4 h-4" /> Go back</button></BlurFade>
                                    </BlurFade>}
                        ...
                        <AnimatePresence>
                            {authStep === 'confirmPassword' && isSignUp && <BlurFade key="confirm-password-field" className="w-full">
                                <div className="relative w-full">
                                    <AnimatePresence>
